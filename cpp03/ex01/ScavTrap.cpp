...
/*   Updated: 2021/03/30 22:51:38 by hroh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	print_border(SCARVTRAP);
	std::cout << "<" << this->name << "> Attempted range attack to <" 
...
{
	print_border(SCARVTRAP);
	std::cout << "<" << this->name << "> Attempted melee attack to <"
...
void	ScavTrap::challengeNewcomer(FragTrap &challenger)
{
...
bool	ScavTrap::enemy_fight_to_challenger(FragTrap &challenger, ScavTrap &scarv)
{
	int	ran;
	unsigned int damage = 0;
	...
	while (challenger.get_hp() > 0 && scarv.hp > 0)
	{
		...
		{
			damage = challenger.vaulthunter_dot_exe(scarv.name);
...
void	ScavTrap::enemy_10_kill_challenges(FragTrap &challenger)
{
	...
	for (i = 0; i < 10; i++)
	{
		fail = enemy_fight_to_challenger(challenger, *horde[i]);
		if (fail == true)
			break ;
	}
	print_border(FRAGTRAP);
...
void	ScavTrap::boss_kill_challenges(FragTrap &challenger)
{
	bool		fail;
	ScavTrap *boss = new ScavTrap("Boss");

	boss->set_spec(1000, 5, 5);
	fail = enemy_fight_to_challenger(challenger, *boss);
	print_border(FRAGTRAP);
	if (fail == true)
	{
		std::cout << YELLOW "Challenge failure..." DEFAULT << std::endl;
